...
import { isLocationActive, isStorageLocation, LOCATION_TYPE_LABELS, operationalRoleFor, ROLE_LABELS } from "@/lib/types";

export default function InventoryTransfers() {
  const loc = useLocation();
  const inventory = useAppStore((s) => s.inventory);
  const products = useAppStore((s) => s.products);
  const movements = useAppStore((s) => s.movements).filter((movement) => movement.type === "traslado" || movement.type === "devolucion");
  ...
  const isReturnFlow = Boolean(destinationLocation && isStorageLocation(destinationLocation) && selectedResponsibleNames.length > 0);
  ...
  return (
    <>
      <PageHeader
        title="Traslados"
        subtitle={isReturnFlow ? "Cuando vuelve a almacen, se registra como devolucion" : "Mueve pares completos o unidades entre ubicaciones"}
      />
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="rounded-2xl bg-card border p-5 space-y-4">
          ...
          <div className="rounded-xl bg-secondary/40 px-3 py-2 text-xs text-muted-foreground">
            ...
            {isReturnFlow && selectedResponsibleNames.length > 0 ? ` Esta operacion quedara como devolucion de ${selectedResponsibleNames.join(", ")}.` : ""}
          </div>
        ...
        <div className="rounded-2xl bg-card border overflow-hidden">
          <div className="px-4 py-3 border-b font-display font-bold">Historial de movimientos</div>
