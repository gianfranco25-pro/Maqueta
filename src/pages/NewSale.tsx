    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-6">
        ...
        <div className="space-y-4 lg:sticky lg:top-20 lg:self-start">
          <div className="rounded-2xl bg-card border border-border/60 p-4 space-y-3">
            <h2 className="font-display font-bold">Pago</h2>
            {payments.map((p, i) => (
              <div key={i} className="rounded-xl border border-border p-3 space-y-2">
                <div className="flex gap-2 items-center">
                  ...
                  <Input type="number" value={p.amount} onChange={(e) => updatePayment(i, { amount: +e.target.value })} className="w-28" />
                  {payments.length > 1 && (
                    <Button size="icon" variant="ghost" onClick={() => removePayment(i)}><Trash2 className="size-4" /></Button>
                  )}
                </div>
                {p.method === "tarjeta" && (
                  <label className="flex items-center justify-between text-xs gap-2 bg-secondary/50 rounded-md px-2 py-1.5">
                    <span className="flex items-center gap-1.5"><CreditCard className="size-3.5" />Recargo {settings.cardSurchargePct}%</span>
                    <input
                      type="checkbox"
            ...
            <Button variant="outline" size="sm" onClick={addPayment} className="w-full">
              <ArrowLeftRight className="size-4 mr-1" /> Pago mixto / dividir
            </Button>
          </div>

          <div className="rounded-2xl bg-foreground text-background p-4 space-y-2 shadow-lg">
            <div className="flex justify-between text-sm"><span className="opacity-70">Subtotal</span><span>{fmtMoney(subtotal)}</span></div>
            {surchargeTotal > 0 && (
              <div className="flex justify-between text-sm"><span className="opacity-70">Recargo tarjeta</span><span>+{fmtMoney(surchargeTotal)}</span></div>
            )}
            <div className="flex justify-between font-display font-extrabold text-2xl pt-1 border-t border-background/20"><span>Total</span><span className="text-accent">{fmtMoney(total)}</span></div>
            <div className="flex justify-between text-sm"><span className="opacity-70">Pagado</span><span>{fmtMoney(paid)}</span></div>
            <div className={`flex justify-between text-sm font-semibold ${Math.abs(remaining) < 0.01 ? "text-success" : "text-critical"}`}>
              <span>Falta</span><span>{fmtMoney(Math.max(0, remaining))}</span>
            </div>
            {exceedsDiscount && (
